...
try { BANS = new Set(await fs.readFile('./blacklist.txt').toString().split('\n')) } catch (e) { }
...
wss.on('connection', async function(p, {headers, url: uri}) {
  ...
  p.on('message', async function(data) {
		...
		if(cd > NOW){
			//reject
			console.log("rejecting the data :( ...")
		...
		{
			console.log("accepting the data :) ...")
		}
		//accept
		CHANGES[i] = c		
		...
		newPos.push(i)
		newCols.push(c)
		console.log("done pushing pixel data")
  })
